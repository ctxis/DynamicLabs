output "details" {
    value = [ for system in var.systems_map : {
        ...
        "public_ip" = system["public_ip"] == true ? azurerm_public_ip.ubuntu_server[system["name"]].ip_address : null
        "private_ip" = azurerm_network_interface.ubuntu_server[system["name"]].private_ip_address
