...
resource "azurerm_public_ip" "windows_desktop" {
...
resource "azurerm_network_interface" "windows_desktop" {
    ...
    ip_configuration {
        ...
        public_ip_address_id          = each.value["public_ip"] == true ? azurerm_public_ip.windows_desktop[each.value["name"]].id : null
    }
}