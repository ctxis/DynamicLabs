...
resource "azurerm_virtual_machine" "windows_server_2016" {
    ...
    os_profile {
        ...
        custom_data    = file("Microsoft/Windows/Server_2016/files/ConfigureRemotingForAnsible.ps1")
    }
...
resource "azurerm_network_interface_security_group_association" "windows_server_2016" {
    for_each = var.systems_map
    network_interface_id      = azurerm_network_interface.windows_server_2016[each.value["name"]].id
