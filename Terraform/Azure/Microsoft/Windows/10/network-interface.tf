...
resource "azurerm_public_ip" "windows_10" {
...
resource "azurerm_network_interface" "windows_10" {
    ...
    ip_configuration {
        ...
        public_ip_address_id          = each.value["public_ip"] == true ? azurerm_public_ip.windows_10[each.value["name"]].id : null
    }
}