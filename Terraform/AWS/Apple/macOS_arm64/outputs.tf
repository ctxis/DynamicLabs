output "details" {
    value = [ for system in var.systems_map : {
        ...
        "private_ip" = aws_instance.macos_arm64["${system["name"]}"].private_ip
        "public_ip" = aws_instance.macos_arm64["${system["name"]}"].public_ip == null ? null : aws_instance.macos_arm64["${system["name"]}"].public_ip
